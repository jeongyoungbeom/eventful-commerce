...
[판매자] 상품 등록 (product-service)
  └─► product-events(PRODUCT_REGISTERED) 발행
        └─► order-service: ProductReadModel 저장 + Redis 재고 초기화

[구매자] 다상품 주문 생성 (order-service)
  └─► ProductReadModel 로컬 조회 (서비스 간 HTTP 호출 없음)
  └─► Redis Lua 스크립트로 상품별 수량 예약 (원자적)
  └─► 재고 부족 상품은 failedItems로 응답, 가능한 상품만 주문 생성
  └─► Order → SellerOrder → OrderItem 구조로 판매자별 주문 분리
  └─► order-events(ORDER_RESERVED) 발행
        └─► payment-service: 결제 레코드 생성

[PG사 웹훅] POST /payments/webhook
  └─► payment-events(PAYMENT_COMPLETED) 발행
        └─► order-service: 예약 재고 확정 + ORDER_CONFIRMED 발행
              └─► shipping-service: SellerOrder별 배송 생성
              └─► settlement-service: SellerOrder별 정산 생성 (PENDING)
        └─► notification-service: 각 이벤트마다 Telegram 알림 전송

[구매자] 주문/판매자 주문 취소
  └─► order-events(ORDER_CANCELED) 발행
        └─► payment-service: PaymentRefund 생성 + PAYMENT_REFUNDED 발행
              └─► settlement-service: 환불 금액만큼 정산 차감
...
| **shipping-service** | 8083 | SellerOrder별 배송 생성 및 상태 처리 |
...
fun orders(request: OrdersRequest, userId: UUID): OrderResponse {
    val order = ordersRepository.save(order)
...
매일 자정 전날 생성된 PENDING 정산을 1000건 단위 청크로 처리합니다. 정산은 SellerOrder 단위로 생성되며, 취소/부분 환불 이벤트가 들어오면 상품 합계·수수료·정산 금액에서 환불분을 차감합니다. 수수료율은 환경별 `application.yml`로 관리합니다.
...
> 관련 테스트: `scripts/test-02-stock-oversell-traffic.sh`
...
> 관련 테스트: `scripts/test-04-payment-event-idempotency.sh`
...
# 헬스 체크
curl http://localhost/actuator/health  # Nginx → api-gateway

# 컨테이너 내부 네트워크에서 개별 서비스 확인이 필요할 때
docker compose exec api-gateway curl -s http://localhost:8080/actuator/health
docker compose exec order-service curl -s http://localhost:8081/actuator/health
...
> Docker/Nginx 기준 외부 진입점은 `http://localhost` 또는 서버 IP입니다.
> 모든 비즈니스 API는 api-gateway를 통해 `/api/` 접두사로 호출합니다.
...
GET /api-docs/order
GET /api-docs/payment
GET /api-docs/settlement
GET /api-docs/notification
```
...
```bash
# 상품 등록 (multipart/form-data, 등록 시 Redis 재고 자동 초기화)
POST /api/products
request={ "name": "상품명", "description": "설명", "price": 10000, "stock": 100, "category": "ELECTRONICS" }
images=@image.jpg  # 선택
...
```bash
# 다상품 주문 생성 (userId는 JWT에서 추출, 금액은 서버 계산)
POST /api/orders
{
  "items": [
...
# 응답은 판매자별 SellerOrder와 실패 상품을 함께 반환
{
  "orderId": "uuid",
  ...
  "sellerOrders": [
    {
      ...
      "items": [
        { "productId": "uuid-1", "productName": "상품명", "quantity": 2, "unitPrice": 10000 }
      ]
    }
  ],
  "failedItems": [
    { "productId": "uuid-2", "reason": "INSUFFICIENT_STOCK", "requestedQuantity": 1, "availableQuantity": 0 }
  ]
}
...
# 판매자 주문 부분 취소
POST /api/orders/{orderId}/seller-orders/{sellerOrderId}/cancel
```
...
# 주문 취소 후 payment-service는 PaymentRefund를 생성하고 PAYMENT_REFUNDED 이벤트를 발행
```
...
# status: PENDING, PARTIALLY_REFUNDED, REFUNDED, CONFIRMED, PAID
...
SELLER_TOKEN=$(curl -s -X POST $BASE/auth/login/seller \
  -H "Content-Type: application/json" \
...
PRODUCT_ID=$(curl -s -X POST $BASE/products \
  ...
  -F 'request={"name":"테스트상품","description":"설명","price":10000,"stock":100,"category":"ELECTRONICS"};type=application/json' \
  | jq -r '.productId')

sleep 3  # product-events 전파 대기

# 3. 구매자 회원가입 & 로그인
curl -X POST $BASE/auth/signup/user \
  -H "Content-Type: application/json" \
  -d '{"email":"buyer@test.com","password":"Test1234!","name":"구매자"}'

BUYER_TOKEN=$(curl -s -X POST $BASE/auth/login/user \
  -H "Content-Type: application/json" \
  -d '{"email":"buyer@test.com","password":"Test1234!"}' | jq -r '.accessToken')

# 4. 주문 생성 (성공 상품은 orderId, 실패 상품은 failedItems로 응답)
ORDER_ID=$(curl -s -X POST $BASE/orders \
  -H "Authorization: Bearer $BUYER_TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"items\":[{\"productId\":\"$PRODUCT_ID\",\"quantity\":1}]}" | jq -r '.orderId')
...
├── common-outbox/          # Outbox Publisher + 이벤트 Payload 정의
├── common-idempotency/     # 멱등성 처리 공통 모듈
├── api-gateway/            # Spring Cloud Gateway MVC, JWT 검증, Swagger 집계 (8080)
├── user-service/           # 인증/인가, 회원 관리 (8085)
├── product-service/        # 상품 관리 (8086)
├── order-service/          # 다상품 주문, SellerOrder, 재고 예약, ProductReadModel (8081)
├── payment-service/        # 결제, PG 웹훅, 환불 이벤트 (8082)
├── shipping-service/       # SellerOrder별 배송 (8083)
├── notification-service/   # Telegram 알림 (8084)
├── settlement-service/     # SellerOrder별 정산, 환불 차감, Spring Batch (8087)
├── nginx/                  # Nginx 리버스 프록시 설정
├── scripts/                # DB/Redis 초기화 및 E2E/동시성 테스트 스크립트
├── docker-compose.yml
